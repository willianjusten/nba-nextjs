import { addDays, addYears, format, getMonth, getYear, parseISO, subDays } from "date-fns";
import { COVID_MONTH_END, COVID_YEAR, DATE_LINK_FORMAT, REGULAR_MONTH_END } from "@/app/constants";
...
export const getLeagueYear = (date: Date) => {
  if (getYear(date) === COVID_YEAR) {
    ...
    return getMonth(date) > COVID_MONTH_END ? getYear(date) : getYear(addYears(date, -1));
  } else {
    // if it's after july, it's a new season
    return getMonth(date) > REGULAR_MONTH_END ? getYear(date) : getYear(addYears(date, -1));
  }
};
